...
use crate::{IpAddrType, ProxyResult};
...
pub async fn resolve(addr: &str, ip_addr_type: IpAddrType) -> ProxyResult<Vec<DnsResolutionInfo>> {
    // IDNA parsing
...
pub async fn resolve_single(addr: &str, ip_addr_type: IpAddrType) -> ProxyResult<DnsResolutionInfo> {
    let rresults = resolve(addr, ip_addr_type).await?;
...
// Tests are tmp ignored as we switch to AsyncResolver. 
// #[cfg(test)]
// mod tests {
//     use super::*;

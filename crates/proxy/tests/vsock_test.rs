...
use proxy::{vsock::VsockProxy, IpAddrType, vsock::VSOCK_PROXY_PORT};

fn vsock_connect(port: u32) -> VsockStream {
    let sockaddr = VsockAddr::new(VSOCK_PROXY_PORT, port);
...
fn test_proxy_vsock_to_tcp_connection() {
    ...
    let client_handle = thread::spawn(move || {
        let mut stream = vsock_connect(VSOCK_PROXY_PORT);
